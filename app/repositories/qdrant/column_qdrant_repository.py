...
class ColumnQdrantRepository:
    ...
    async def upsert(self,ids:list[str],payloads:list[dict],embeddings:list[list[float]],batch_size:int=10):

        points:list[PointStruct]=[PointStruct(id=id,vector=embedding,payload=payload) for id,payload,embedding in zip(ids,payloads,embeddings)]
